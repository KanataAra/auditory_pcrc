...
def execute(df,op):
    ...
    if parallel>=1:
        ...
        command="parallel -a %s" % file_sh
        #print(command)
        subprocess.call(command.split())

    df2 = pd.read_csv(file_tmp,sep=separator,names=columns)
    ...
    df2.to_csv(file_opt, index=False, mode='a', header=False)
    return df2
