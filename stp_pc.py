...
def plot_colormap():
    ...
    pcolor(X1, Y1, np.log(Y.T+0.0000001))
    colorbar()
    show()

    #X,Y=meshgrid(x,y)
    #pcolor(X, Y, np.log(Y1.T+0.1))
    pcolor(X1, Y1, np.log(D.T+0.0000001))
    colorbar()
    show()
...
#    T1 = 10000 # length of training data
    ...
    for i in range(Ntest):
        ...
        SUM=np.zeros(3)
        for j in range(0,T3):
#            SUM[0] += np.sqrt(mean_squared_error(Y[j,:], D[j,:]))
            SUM[0] += np.linalg.norm(Y[j,:] - D[j,:])
            print(SUM[0])
        for j in range(T0+T3,T0+2*T3):
            SUM[1] += np.linalg.norm(Y[j,:] - D[j,:])
        for j in range(T0+2*T3,T0+3*T3):
            SUM[2] += np.linalg.norm(Y[j,:] - D[j,:])
