...
def fileout():
    ...
    str="%d,%d,%f,%f,%f,%f,%f,%f,%f,%f,%f\n" % (id,seed,sm,alpha_r1,alpha_r2,gamma_r1,gamma_r2,alpha_nmf,l1_ratio_nmf,e1,e2)
    print(str)
...
def config():
    global id,seed,alpha_r1,alpha_r2,gamma_r1,gamma_r2,alpha_nmf,l1_ratio_nmf,sm
    args = sys.argv
    for s in args:
        id = arg2i(id,"id=",s)
...
def phi(n, sigma):
    ...
    # exp(-(n/sigma)^2)
    """
    return np.exp(-(n/sigma)**2)

def calc_spike(temporal_position, sigma, amp):
    n = np.arange(T1)
    spike = amp * phi(n - temporal_position, sigma)
    return spike*0.5
